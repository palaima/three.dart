    ...
    for (var k = 0; k < data['hierarchy'][h].keys.length; k++) {
      ...
      if (data['hierarchy'][h]['keys'][k]['rot'] != null &&
        ...
        data['hierarchy'][h]['keys'][k]['rot'] =
            new Quaternion.array(quat.map((e) => e.toDouble()).toList());
      }
    }

    // prepare morph target keys

    if (data['hierarchy'][h]['keys'].isNotEmpty &&
      ...
      for (var k = 0; k < data['hierarchy'][h]['keys'].length; k++) {
        for (var m = 0;
            ...
            m++) {
          var morphTargetName =
              data['hierarchy'][h]['keys'][k]['morphTargets'][m];
      ...
      for (var k = 0; k < data['hierarchy'][h]['keys'].length; k++) {
        ...
        for (var morphTargetName in usedMorphTargets) {
          for (m = 0;
              m < data['hierarchy'][h]['keys'][k]['morphTargets'].length;
              m++) {
            if (data['hierarchy'][h]['keys'][k]['morphTargets'][m] ==
                morphTargetName) {
...
List<Bone> parse(Object3D root) {
  parseRecurseHierarchy(Object3D root, List hierarchy) {
    hierarchy.add(root);
