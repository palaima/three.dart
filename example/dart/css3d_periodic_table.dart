...
void init() {
  camera = new PerspectiveCamera(40.0, window.innerWidth / window.innerHeight, 1.0, 10000.0)
    ..position.z = 3000.0;
  ...
  for (var i = 0; i < table.length; i += 5) {
    var element = new DivElement()
      ..className = 'element'
      ..style.backgroundColor = 'rgba(0,127,127,${rnd.nextDouble() * 0.5 + 0.25})';
    ...
    element.append(new DivElement()
      ..className = 'details'
      ..innerHtml = table[i + 1] + '<br>' + table[i + 2]);

    var object = new CSS3DObject(element)
      ..position.x = rnd.nextDouble() * 4000 - 2000
      ..position.y = rnd.nextDouble() * 4000 - 2000
      ..position.z = rnd.nextDouble() * 4000 - 2000;
    ...
    targets['table'].add(new Object3D()
      ..position.x = (table[i + 3] * 140.0) - 1330.0
      ..position.y = - (table[i + 4] * 180.0) + 990.0);
  }
  ...
  for (var i = 0; i < objects.length; i++) {
    targets['grid'].add(new Object3D()
      ..position.x = ((i % 5) * 400.0) - 800.0
      ..position.y = (-((i / 5).floor() % 5) * 400.0) + 800.0
      ..position.z = ((i / 25).floor()) * 1000.0 - 2000.0);
  }
  ...
  querySelector('#table').onClick.listen((e) => transform(e.target, targets['table'], 2000));
  querySelector('#sphere').onClick.listen((e) => transform(e.target, targets['sphere'], 2000));
  querySelector('#helix').onClick.listen((e) => transform(e.target, targets['helix'], 2000));
  querySelector('#grid').onClick.listen((e) => transform(e.target, targets['grid'], 2000));

  transform(window, targets['table'], 2000);
